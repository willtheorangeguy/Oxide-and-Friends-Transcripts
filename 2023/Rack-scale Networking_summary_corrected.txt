• Discussion of a previous episode of "On the Metal" where they shared their favourite moments
...
• Mention of an upcoming blog post on the development of Oxide's switch
...
• Programmable switches, such as the Torino 2 from Intel, support speeds of up to 12 terabits per second
...
• Torino ASIC allows for dynamic resource allocation and reconfiguration
• The Torino simulator is used for development and testing, but has limitations
• Building a software stack for Torino requires a system-level approach
• The team is working on developing drivers, operating systems, and APIs for Torino
...
• Comparison of using the behavioural model version 2 (BDM 2) versus compiling P4 code directly
...
• OPTED (distributed virtual switch) lives in the kernel and leverages NIC's to handle traffic
• Gimlets (servers) can interact with Torino-based switches through a PCIe peripheral
...
• Examples of complex routing and Pub Sub systems used by companies like Facebook/Meta to manage their networks
...
• Implementing a p4 compiler and integrating it with Trace for debugging and visibility
...
• Tunnelling protocols (Fireguard, Geneva, VLAN) for remote site connections
• Delay-driven multipath (DDM) routing protocol for scalable routing
• Oxide network architecture (underlay and overlay networks) with ipv6 underlay and GENEVA overlays
...
• Comparison of routing protocols (RIFT, RCMP) and drawbacks of current implementations
• Measurement and analysis of network traffic matrices for routing protocol optimization
• Combining microloan balancing and congestion control approaches from 2017 and 2020 papers
...
• Concerns about Smartness as a solution, considering the complexity of network stacks and the need for a more programmable approach
• Potential for Smartness to provide value in specific scenarios, such as line rate encryption on a per-flow basis
• Limitations of current Smartness, including speed and programmability
...
• Need for a clear understanding of the system's behaviour and the ability to revisit and adjust decisions as needed
