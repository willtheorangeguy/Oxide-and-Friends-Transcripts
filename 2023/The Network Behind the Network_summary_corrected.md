...
• Mention of a 2019 talk by Rick Alter on a BMC exploit, "BMC Anywhere"
• Explanation of the vulnerability and its potential consequences
• Discussion of the use of CSI (Network Configuration and Services Interface)
• NIC (Network Interface Card) features and requirements
• CSI (Network Channel Sideband Interconnect) protocol and its challenges
• Reset and power domain concerns with CSI
• Complexity of NIC's with multiple power rails, CPU cores, and features
• Difficulty in understanding vendor implementation of CSI and lack of documentation
• Uncertainty and frustration with CSI's potential to work correctly
• Comparison of CSI to traditional OCP (Open Compute Project) designs
• Discussion of "CSI or bust" catchphrase and its implications.
• The difficulties of implementing CSI (Network Control and Status Interface) due to power management complexities and conflicting design decisions.
• The decision to pursue a cable backplate and blind mating into it, which reduced the argument for CSI.
...
• Industry pseudo-standard SEMI and use of breakout chips to achieve 48 ports
...
• Alternative solution using Microchip's switch ASIC with SDM AI (Serial Gigabit Independent Interface)
• Use of SDM AI for differential signalling, allowing for AC coupling and reduced need for magnetics
...
• The team had to find a solution to connect the service processor to two independent switches using a single RMI connection
• They used a PHY part from Microchip to translate the RMI interface to SEMI and 100 FX interfaces
...
• Moving the Switch chassis to sit in another chassis across a cable, exposing a clean SGM II interface to the outside world.
• Exploring the possibility of connecting a microcontroller with an SEMI interface or a small switch with 2 SGM II interfaces and an RMI link.
...
• The chip has a mode where groups of 4 Series are ganged together, which causes issues if not properly configured.
...
• Comparison with other network configurations, including Trace and CSI alternative
...
• Discussion of the use of John's work on the Humid console and its non-lossy, non-faithful serial console proxy
