...
• Pathological behaviour in networking, specifically when crossing VPCs
...
• Collaboration among team members to solve the issue, including Will, Alan, Vaughn, and Trey
...
• The traffic was hair pinning through the upstream device, which was unusual and led to the discovery of the issue.
• The Oxide Rack's implementation of VPCs and Geneva packet encapsulation was a key factor in the issue, causing VPC-to-VPC traffic to leave the rack.
• The current implementation of the rack's virtual switch interface (OPTED) causes VPC-to-VPC traffic to leave the rack by default, but this can be a desirable behaviour for customers who want to put policy around tenant-to-tenant communication.
• The team determined that traffic leaving the rack is pathological, but this is a known and intentional behaviour, and implementing hair pinning inside the rack would be a feature addition.
...
• Investigation of control plane polices and counters
• Realization that the packets are being classified incorrectly due to a potential bug in the Nexus switch
• A bug is causing unexpected behaviour, and the team is trying to troubleshoot the issue
...
• Cisco Nexus IP redirects feature is enabled by default but recommended being disabled due to potential issues
...
• Comparison of various network infrastructure CLI's and their differences
...
• Discussion of the "lowercase an" agile development process, focusing on closing the loop between improvement and implementation.
...
• Analysis of the behaviour of the Cisco device, with Bryan Can trill noting that the behaviour itself is not unreasonable, but the combination of behaviours leads to a pathological system.
• Discussion of the difficulties of integrating technology from different vendors, with Devon Carver noting the prevalence of workarounds and the challenges of understanding how complex systems work.
