...
• Andy's mention of being compared to Mystery Science Theatre 3000 (MST3K)
• Discussion of the hosts' goal to be the "car talk of technology" (click and clack of tech)
• Comparison of the hosts' conversational tone and humour to a "journal club" version of MST3K
...
• The use of deep learning, specifically cerebral, which allows for open and accessible models
• The speed and responsiveness of deep learning models, such as deep seek on cerebral, which can process and respond to questions instantaneously
• The feeling of "living a thousand lives" with the rapid response time of deep learning models
• Performance of Deep Seek and comparison to Cerberus's own models
• Discussion of the release of Lama and its impact on Cerberus
• Comparison of Cerberus's previous exotic hardware to more mainstream options
...
• Cerebral' technological innovation is the development of a wafer scale engine, the largest computer chip ever made
...
• The wafer scale engine addresses these needs with 900,000 cores, 125 petalous of sparse FP16 compute, and all on-chip SRAM memory
...
• The memory capabilities of the Cerberus chip, specifically its 44 gigabytes of on-chip SRAM
• The speed advantages of the Cerberus chip in processing large language models (LLMs) due to its ability to store weights in SRAM and stream them through the ALU
• The limitations of GPU architectures in processing LLMs due to memory bandwidth limitations
• Cerberus achieves a "GPU impossible" level of performance for certain problems
...
• Current GPU unit of memory scaling is limited by NV Link, which maxes out at 8 GPUs
• New generation of GPUs may offer higher memory scaling capabilities, but efficiency is still unknown
• Grebes' programming model is distinct from CUBA, using a custom microcode instruction set and compiler
...
• The software developer ecosystem has evolved beyond CUBA
• Cerberus allows users to run PyTorch models with ease and high performance
• A marketing claim by NVIDIA is questioned, comparing CUBA to a 10,000-year lead
...
• The introduction of LLAMA has helped hardware companies like Cerebral find product market fit and compete in the market
...
• Metaphor of "dead walking" for closed models, implying their no longer viable
...
• Reference to Cerberus, which is mentioned as being very fast and producing a "manic" output
• Analysis of the model's behaviour, likened to self-doubt and neuroticism, and comparison to a scene from Breaking Bad
...
• Motivations behind OpenAI's Deeper and Llama being open-source are discussed, including giving back to the community and commoditizing competitors
...
• ChatGPT being used to write next door posts to troll neighbours
...
• High-performance computing and traditional physics-based modelling and simulation
...
• Collaboration with the Department of Energy to optimize Cerebral architecture for molecular dynamics simulations
• Achieving record-breaking speeds in molecular dynamics simulations, outperforming the world's fastest supercomputer
• The need for new neural network architectures optimized for wafer scale and Cerebral architecture
...
• DeepSeek discussion and Cerebral' work
