...
• The podcasters discuss their failure to discuss the async cancellation issue in a previous episode, with Rain Pahari's blog entry on the topic being mentioned
...
• The issue is related to Orb connection pooling and Tokyo task activity
• The problem is similar to a previous issue debugged in June, where the Tokyo scheduler was not picking up a runnable task
• CORE (CORE pool) has probes and internal bookkeeping, which seems to be functioning normally
• CORE's behaviour is not matching expectations, with messages not being received from senders
• The team has ruled out several theories, including observing the wrong CORE pool and CPU issues
...
• The team is struggling to understand the behaviour, with Bryan Can trill comparing it to a mystery to be solved
...
• The team is discussing how it's possible for this behaviour to occur and are considering the possibility that there's an oversimplification in their mental model of the MPSC channel
• Discussion of a mysterious problem with a channel being empty despite many tasks trying to send to it
• Use of Trace and dynamic instrumentation to collect core dumps and debug the problem
• Analysis of the core dumps using Hydra and source-assisted reverse engineering
...
• Use of Loom, a concurrency model checker, to simulate possible interleaving of events
...
• The team has realized that linked list blocks can be reused and potentially initialized
...
• The historical example of the MEET scheduling model in Polaris, which had problems with blocking threads
• The introduction of the SIGQUIT signal to mitigate the problems of the MEET model
...
• Bryan Can trill's praise for Dave Pacheco's responses to comments on hacker news
...
• Refactoring of Core to make it more defensive has strengthened the project and given confidence in its future
• Upcoming episode on async cancellation will be discussed
• Upcoming episode will feature a "scrum of founders" discussing the book "Founder vs. Investor" by Elizabeth Kalman and Jerry Newman
• Bryan Can trill apologizes for missing the Rust Conference due to not forcing the guests to attend