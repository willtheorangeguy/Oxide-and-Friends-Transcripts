...
• IMDB (briefly mentioned)
• Debugger circuit (podcast appearances)
• Debugger history (MDB introduced in Polaris 7)
...
• Influence of MDB on later debugging tools, such as grab and crash
...
• Development of the "colon" command for iterating over linked lists
...
• Explanation of IMDB and its purpose
...
• IMDB's emergence as a useful tool for debugging and insight into system behaviour
• Ground rules for contributing to IMDB, including not wanting people to hesitate to add useful features
...
• IMDB (Open MDB) has been helpful for the company, but its purpose is not to create as many debuggers as possible
• IMDB is a common engineering approach across the company, not directly inspired by MDB or Humility
• Humility and IMDB are "blood cousins" in terms of their ancestry and functionality
...
• Reconfigurator is a critical system for managing changes to the control plane, and IMDB has allowed the company to ship incomplete but useful pieces of the system for support and development operations.
• The importance of IMDB (Online MDB) in incrementally developing and deploying system components
...
• The discussion of MDB commands, including the "colon stream" and "flip one" commands
• A story about a bug in the cache on a Sun 4m system that allowed a program to branch to an arbitrary location, leading to humorous and absurd outcomes
• Discussion of a bug that affected the cache and caused data loss
• Explanation of how the bug was solved by turning off three-quarters of the cache
• Mention of a lesson learned from the bug about the importance of the cache
• Story of how the bug led to a change in the company's sales approach regarding the cache
• Explanation of the "flip one" command and its uses
• Discussion of the ASCII art in IMDB
• Explanation of the equals j format character in IMDB
• Discussion of the importance of debugging tools for system development and implementation.
• The development of the IMDB system sped up the development of other components
• IMDB is a CLI program that provides a simple interface to access services in the rack
• Adding new services to IMDB is straightforward due to its connection to internal DNS
• IMDB's ease of use and existing clients have facilitated the development of other components
• IMDB has allowed for a distributed system to reconfigure itself on the fly, reducing the risk of failures
• The use of IMDB has enabled automation of system reconfiguration, reducing the need for human intervention
• IMDB's tooling has enabled quick identification and resolution of issues, such as unexpected behaviour in automated tests
• IMDB has been used to develop new functionality, such as plugging in sensor data from the MGS service.
...
• Use of IMDB commands to print error reports and test system functionality
• Discussion on system demos and the importance of showing system behaviour in failure scenarios
• Analysis of data storage and organization through IMDB commands
• Example of using IMDB to troubleshoot DNS propagation issues
• Importance of software being able to "exonerate" itself by providing affirmative information about its actions
• Discussion on the benefits of using IMDB to quickly identify and troubleshoot issues
• Using ODB to modify the state of the system
• IMDB's dual purpose: observing the system and making well-defined changes
• Comparison to MDB and Trace's destructive options
...
• The benefits of investing in tools that clearly print out system state and behaviour
...
• IMDB's impact and usefulness in various demos
• Debugger-driven development and IMDB's role in it
• The split between pre-planned and post-debugging IMDB commands
• IMDB's availability and ease of use as a built-in tool
• Using IMDB in simulated environments for testing and debugging
...
• Mention of IMDB (Open Machine Debugging Book) and its value to the team
...
• Bryan Can trill is trying to negotiate a release document to avoid further action
