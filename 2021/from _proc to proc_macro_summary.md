...
• Brian Cantrell's Twitter profile and his identification as the number 1 fan of someone with a similar name
...
• The development of libwatchmalloc and the challenges of implementing watchpoints in the Solaris operating system
• The importance of debugging infrastructure and the contribution of Roger Faulkner to this field
• The development of tracing system calls and signals, including the use of ptrace in the early UNIX systems.
• Ptrace, a historic debugging interface in UNIX, is still used in Linux for debugging and is considered a pain to use.
• Ptrace was originally intended to be used in conjunction with the wait system call, but it was overloading, leading to issues.
...
• The ptrace system call is documented as passing specific integers, and its interface predated the use of macros.
• The group notes that the ptrace system call has been in use for so long that it's difficult to replace, and that computers grow exponentially, making it harder to fix issues over time.
• One of the speakers apologizes to Josh for implementing an emulation of ptrace in branded zones.
...
• Discussion of the ptrace mechanism and its emulation in a Linux system
• Problem of ptrace emulation when switching to a different stack model
• Explanation of the red zone and segmented stacks used by Go
• Discussion of the challenges of emulating a system's behavior, including the need for bug-for-bug compatibility
• Story of the STrace project and its successful implementation using a complex emulation approach
...
• Story about a stolen laptop running Solaris 251 on a PowerPC chip in 1997
...
• Mention of DTrace and its relationship to the d language
...
• Conversation about the DTrace language, its ergonomics, and its domain specificity
• Discussion about the design of DTrace and its high-level layer, including who was involved and what inspired it
• Comparison of DTrace to SQL and other programming languages, including its ease of use and flexibility
• Personal anecdotes about the creation of DTrace and its development process
...
• Control over the compiler's behavior and output
...
• Discussion of DTrace and its implementation in Plan 9 and 9front
• Plan 9 containing the first implementation of a specific feature (likely related to DTrace)
• Praise for the Spitterspace team's progress in maintaining stability
