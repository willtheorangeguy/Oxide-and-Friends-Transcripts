...
• The importance of avoiding errors and manufacturing issues with the FPGA bit stream
• The use of a "board preview" or convection oven to heat the board
• The benefits of using the new bit stream compression method
...
• SALE (Save) is a load generator used to test power supplies and verify their performance under various conditions
• SALE simulates power load steps, voltage changes, and frequency changes to test power supplies' ability to respond and behave
• The device uses Lab view to connect to the power supplies and collect data, which is then analyzed to create graphs and reports
• SALE can test power supplies' behaviour at frequencies up to 300 kHz and can simulate power load steps of up to 200 amps
• The team used SALE to test their power supplies and discovered quirks in the rails, but the two main power supplies passed testing with "flying colours"
• The team is now preparing to attempt their first SP3 power on with a Rome processor loaded into the system
• FPGA behaviour causes issues with I squared c bus
• Level shifters on the bus have undefined behaviour when powered off
...
• Discussing the importance of protecting the SALE (SP3) and its irreplaceable value
• Describing the team's efforts to troubleshoot the system, including testing and reworking wires
• Identifying the source of the issue as a bus scan error in Power Navigator
• Describing the system's response to the error, including bus resetting and undefined behaviour
• Discussion of a processor malfunction, initially suspected to be dead
• Troubleshooting of an I2C bus transaction issue
• Explanation of Power Navigator software's behaviour in response to synchronization loss
...
• Discussion of Power Navigator and PM Bus communication
• Concerns about controlling voltages with PM Bus and potential for processor damage
• Exploring ways to prevent over-voltage or under-voltage with PM Bus
• Open-sourcing Hubris and Humility software for PM Bus reporting
...
• Amusing comparison of the board's behaviour to a scene from Beverly Hills Cop
...
• The discovery of a voltage holdback issue caused by a layout error
...
• Discussion of perfect power margin and its implications
• Mention of SPI 2 traffic and its relation to power management
• Introduction of VOTE (Voltage On The Fly) and its purpose in voltage change requests
• Realization that the power tool was not sending VOTE packets as expected
...
• Troubleshooting a problem with an TDI chip and a dongle
