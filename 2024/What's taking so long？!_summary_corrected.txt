...
• Personal experiences with compile times, including working on the Polaris kernel at Sun Microsystems
...
• Cargo's features causes issues with rebuilding dependencies when multiple crates use the same dependency with different features.
• A crate like SYN can have multiple features, leading to a combinatorial explosion of possible feature sets.
• This can cause all dependent crates to be rebuilt, leading to many unnecessary rebuilds.
• Mozilla Central Repository uses a workspace hack crate to solve this problem, but it requires manual management.
• Cargo Harari is an automated workspace hack manager that resolves this issue by automatically adding build configurations to the workspace.
• Improved build times
• Cargo Atari's limitations
...
• Cargo's behaviour with panic handling in proc macros and build scripts
• Tradeoff between dev and prod builds due to panic handling divergence
• Optimization and speed up achieved through changes to build process
...
• A discussion of the Sturdy crate and its limitations, and the exploration of alternative crates
• The flexibility and usefulness of Sturdy (or similar tools) vs. other approaches like VERDI
...
• Cost reduction strategies for VERDI ecosystem
• Concerns about committing to VERDI and potential future changes
• Investigation of linker issues and performance
• Analysis of build times and developer velocity experience
• Comparison of different linkers (LD, LLD, mould) on various operating systems
• Results showing mould as a significant performance improvement in certain scenarios.
• Discussion about mould, a tool for integrating Rust code
• Comparison of mould's performance to other build tools, such as Lumps LD
...
• The speaker compares Buck to other build systems, including Starter, which is a DSL subset of Python used by Buck and Basil
• The speaker reflects on their experiences with build systems, including scones and Off, and how they can be frustrating to work with
...
• Analysis of various build systems, including make, Basel, Buck, and Excel
...
• Monodic build systems and their design principles
