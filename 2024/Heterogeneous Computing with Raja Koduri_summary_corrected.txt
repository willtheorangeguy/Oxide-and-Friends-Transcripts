...
• Raja Moduli is invited to explain the differences between CPUs, GPUs, FPGAs, and ASICs
• Raja's background in silicon and chip design is discussed, and he is asked to share his experiences in the field
• Raja Moduli's early interest in computer graphics while in grad school in India
...
• Raja Moduli's experience with S3's pivot to becoming a web company
...
• The success of CUBA due to its one-to-one mapping of programming model to execution model
...
• Raja Moduli uses a transportation analogy to explain the different types of computing resources (CPUs, GPUs, FPGAs)
...
• Raja Moduli also mentions FPGAs' ability to be used as a prototyping tool for new designs and their customizability.
...
• CUBA's advantage in implicit data and compute packing
• Difficulty in achieving performance with non-NVIDIA GPUs due to packing and compatibility issues
• CUBA's "virus" effect and widespread adoption despite non-NVIDIA options
...
• Frequency scaling ended in the early 2000s, and instead, LAW architecture and compiler-based approaches were used to improve performance.
...
• Introduction of the Void (Vector Operating Instruction word) idea and its limitations
...
• The 20-year delay in Intel fixing GCC performance and the resulting impact on LAW processors
...
• The concept of general-purpose GPUs (GP GPUs) and the development of languages like Brook, which aimed to make GPUs usable for non-graphics tasks
• The limitations of early GPU languages, such as CUBA, and the need for a programming model that matches the execution model of the hardware
• The introduction of the NVIDIA g80 architecture and CUBA, which revolutionized the use of GPUs for non-graphics tasks.
• CUBA was initially used as a solution looking for a problem and didn't generate significant revenue for NVIDIA
...
• The need for an open CL like thing on Linux was a major reason AMD's deep learning capabilities lagged
• Raja Moduli discusses the challenges of developing a deep learning ecosystem on Linux, including the lack of Linux driver support and the difficulty of finding kernel-level experts
• Windows had a "weird problem" with compute due to its GUI nature and GPU rendering, making it less favourable for GPU computing
• The GPU BU folks and NVIDIA initially prioritized using GPUs for gaming, but later accommodated the trend towards general-purpose computing
...
• Comparison of the cost of building a 1 megawatt data centre using NVIDIA hardware versus other options
• Discussion of the real-world power infrastructure problems, including wasted power from gigawatt solar farms
• Potential for collocating data centres with solar energy sources to reduce costs and increase efficiency
...
• Raja Moduli's calculation of 2 petalous consuming 1400 watts and the need for 2 kilowatts to run at full capacity
• Efficiency and cost considerations in the GP GPU market
...
• A story about Raja Moduli's experience with a lower spec version of a product in China, which unexpectedly boosted sales due to the need for more units to run the same model
...
• The mention of the 2011 article "Software Eating the World" by Marc Andreessen and its mention of companies like Radio (Angry Birds) as future leaders
