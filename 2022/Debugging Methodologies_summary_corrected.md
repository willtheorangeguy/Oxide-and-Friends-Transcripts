...
• Explanation of the origin of the term "green room" in the context of theatre and performance
...
• Using a nonbankable interrupt (NMI) to trigger a dump
• Alternative methods for generating an NMI, such as shorting pins or using PMI/BMC
...
• Experimentation with modifying the controller configuration register to change the behaviour of the devices
...
• Mention of the tool "Mini" and its limitations as a debugging tool
...
• Debugging a Rust crate that introduced a "UP 2" instruction to produce a crash
• The UP 2 instruction generates a SIGILL signal, which is not ideal for debugging
• The speaker implemented a wrapper around the UP 2 instruction to call core intrinsic::abort instead, which compiles to a branch to a UP 2
• This wrapper was used to sprinkle UP 2s throughout the Rust core crate
...
• The issue involved a hang in the Nablus bootloader, which is a very minimal bare metal system
...
• Triple fault caused by undefined behaviour in a crate
...
• Improvements made to MINI diagnostics and the potential for user feedback
