• Origin of the project and its measurement of backplate cabling
• Explanation of a TDR measurement and its use with a wave pulse
• Benefits and limitations of TDR compared to S-parameter measurement
• Challenges of measuring high-speed signals in a 100 gigabit backplate
...
• Planning and modelling of systems to ensure they work, including use of software and simulations
• Importance of PCB design and layout to minimize signal loss
• Use of field solvers to analyze and optimize PCB traces and via
• Complications of via in PCB design and the need for 3D field solvers to analyze them
• Inductances and capacitances in via and their impact on signal behaviour
• The discussion involves creating a 3D physical piece of art with complex via and circuit board layers
• The challenge of debugging and simulating the system's behaviour, including RX layer coupling to TX layers
...
• The complexity of extracting S parameters from many ports, which can take a significant amount of time and resources
• Challenges in simulating complex physical systems, particularly with high-frequency signals
• Importance of accurate modelling and simulation in avoiding costly mistakes
...
• Discussion of the behaviour of electrons in physical systems and the difficulty of modelling their behaviour accurately
• Importance of understanding the physics of interconnects and their impact on signal transmission
• Effects of loss and frequency-dependent loss on signal transmission and the importance of accurate modelling
• Discussion of standing waves and their impact on signal transmission, including the creation of notches in the signal
• Importance of accurate spacing of via and IC packages in avoiding signal degradation and errors
...
• Comparison of different signalling methods, including PAM 4, PAM 5, and PAM 16
...
• Analyzing signal behaviour at differential via and escape routing
• Using finite elements and wave equations to solve for field behaviour
...
• Time domain refractometry (TDR) analysis: determining discontinuities and impedance
• Analogies to phone lines and cable TV, where TDR devices were used for troubleshooting
• Time Domain Refractometry (TDR) used to measure discontinuities in transmission lines
...
• Issue with the NIC not reaching 28 GPS, but working at 10 GPS
• Analysis of jitter using a Teletype LaCroix scope
...
• Explanation of equalization methods (COLE, DFE, FFE) to boost high frequencies
...
• The speaker highlights a unique feature of the probe station, the ability to adjust the planarity of the probe points without moving the tip.
• Discussion of an open-source application called GL Scope client for oscilloscopes
• Explanation of Lib Scope, an object-oriented library that provides an API to oscilloscopes
...
• Description of the vendor-agnostic C++ API presented by Lib Scope
• Explanation of the benefits of using Lib Scope, including reduced vendor-specific code and increased flexibility.
• The discussion revolves around a high-speed data link using SIGMA (Quad Serial Gigabit Media Independent Interface) and its encoding and decoding processes.
• The link has four lanes, each running at 1.25 GPS, with a round-robin send sequence.
• The encoding process involves sending a 10-bit symbol from each lane, with specific idle characters to maintain sync.
• The signalling within each lane is serial GIMME (Gigabit Media Independent Interface Media Extension), similar to regular gigabit Ethernet.
• A custom filter block was written to work with the SIGMA data, which decodes 8B10B symbols and outputs four additional streams at one quarter of the rate.
...
• Developing a decoded for gigabit Ethernet
...
• Importance of designing PCB stickup to minimize losses
• Trapezoidal effects on traces and their impact on signal transmission
• Enid (Electroless Nickel Immersion Gold) plating and its advantages
• Nickel's poor performance as a conductor for RF signals
• Use of micros trip instead of external traces
...
• The fibre weave effect, where the orientation of the fibreglass weave affects the dielectric constant, can cause issues with signal delay and skew.
• Simulation can capture the effects of the fibre weave, but measurement is also possible.
• Mitigating the fibre weave effect requires careful routing and optimization, and can be a significant challenge.
• The impact of the fibre weave effect can be as little as 1–2 picoseconds, which can be critical in high-speed designs.
• Analyzing the fibre weave effect in simulation can be done by modelling the epoxy and fibreglass weave.
...
• Complex backplate design with multiple connectors and copper connections
