...
• Explanation of challenges with bringing up the Sidecar board, particularly the Torino 2 ASIC
...
• Replacement of Torino on select boards with load slammer
• FPGA control and sequencing of power rails
• Use of simulation models (Sim plus) for initial tuning parameters
...
• Discussion of the heat sink used in the device and its vapour chamber technology
...
• Using Tapton to reduce thermal camera interference and track temperature
...
• Description of the USE 7448 chip and its MIPS processor
...
• Programming an external EPROM with parameters for the Series IP and Torino ASIC
...
• Mention of a custom cable backplate and its development
...
• Troubleshooting a clocking issue with a Toughing device
• Understanding the complexity of the ASIC and the need for Intel's SDK
• Discussing the development process for the VSC 7448 and Toughing devices
...
• Footprint issue with 16-pin DFN packages, CAD error causing rotated pins
...
• The team had trouble with a link between two ports on a chip, leading to a diagnosis of a missing loop back cable
• A software issue with a JSON parser led to missed configuration steps, including analog tuning parameters for Series
